describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    ...
    //access elements of prodItemsData
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    //query a product-item
    const prodItem = await page.$('product-item');
    //grab shadowRoot of that element
    const shadowRoot = await prodItem.getProperty('shadowRoot');
    //await
    const buttonHandle = await shadowRoot.$('button');
    await buttonHandle.click();
    //once you have it, click it and check innertext property of the button
    const innerTextProp = await buttonHandle.getProperty('innerText');
    //once you have the innerText property, use innerText.jsonValue() to get its text value
    const buttonText = await innerTextProp.jsonValue();
    //check to makesure swap occured
  ...
  //should be 20
  const cartCountVal = await cart.getProperty('innerText');
  const cartInnerText = await cartCountVal.jsonValue();
  //check to make sure cart-count is 20
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i<prodItems.length;i++){
      ...
      const buttonInnerTextVal= await buttonInnerText.jsonValue();
      //make sure all buttons say "Remove from cart"
      expect (buttonInnerTextVal).toBe('Remove from Cart');
    }
    const cartCount = await page.$('#cart-count');
    const cartCountVal = await cartCount.getProperty('innerText');
    const cartTotal = await cartCountVal.jsonValue();
    //check cart count
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const prodItems = await page.$$('product-item');
    //loop through all items and remove from cart
    for ( let i = 0; i< prodItems.length; i++){
      const prodItem = prodItems[i];
      const shadowRoot = await prodItem.getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for ( let i = 0; i< prodItems.length; i++){
      ...
      const buttonInnerText = await button.getProperty('innerText');
      const buttonInnerTextVal= await buttonInnerText.jsonValue();
      //check to see if text is correct - should be Add to Cart
      expect(buttonInnerTextVal).toBe("Add to Cart");
    }
    const cartCount = await page.$('#cart-count');
    const cartCountVal = await cartCount.getProperty('innerText');
    const cartTotal = await cartCountVal.jsonValue();
